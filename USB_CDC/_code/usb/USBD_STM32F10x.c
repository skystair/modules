...
/* __packed fallback for armclang if CMSIS compiler header not fully included */
#ifndef __packed
...
static void USBD_EP_HW_Read (uint8_t ep_addr) {
  ...
  ptr_dest = (uint16_t *)(ptr_ep->data + ptr_ep->num_transferred_total);
...
static int32_t USBD_PowerControl (ARM_POWER_STATE state) {
  ...
  switch (state) {
    ...
    case ARM_POWER_FULL:
      ...
      RCC->APB1ENR   |=  RCC_APB1ENR_USBEN;             // Enable USB Device clock
      RCC->APB1RSTR  |=  RCC_APB1RSTR_USBRST;           // Reset USB Device
      { volatile uint32_t dly = 1000; while (dly--); }   // Wait ~1ms (busy, before kernel start)
      RCC->APB1RSTR  &= ~RCC_APB1RSTR_USBRST;           // Release USB Device Reset
...
static int32_t USBD_DeviceConnect (void) {
  ...
  GPIO_PinWrite (RTE_USB_DEVICE_CON_PORT, RTE_USB_DEVICE_CON_BIT, RTE_USB_DEVICE_CON_ACTIVE);
#else
  BCDR |= BCDR_DPPU;                             // Enable DP pull-up (internal)
#endif
...
static int32_t USBD_DeviceDisconnect (void) {
  ...
  GPIO_PinWrite (RTE_USB_DEVICE_CON_PORT, RTE_USB_DEVICE_CON_BIT, !RTE_USB_DEVICE_CON_ACTIVE);
#else
  BCDR &= ~BCDR_DPPU;                           // Disable DP pull-up (internal)
#endif
...
// Setup Packet
      if (val & EP_SETUP) {
        ...
        ptr_dest = (uint16_t *)(setup_packet);
        for (i = 0U; i < 4U; i++) {
          *ptr_dest++ = *ptr_src++;
        }
        setup_received = 1U;
