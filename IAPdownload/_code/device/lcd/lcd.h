...
#define LCD_WR_GPIO_CLK_ENABLE()        do{ RCC->APB2ENR |= 1 << 5; }while(0)

#define LCD_RD_GPIO_PORT                GPIOD
#define LCD_RD_GPIO_PIN                 GPIO_Pin_4
#define LCD_RD_GPIO_CLK_ENABLE()        do{ RCC->APB2ENR |= 1 << 5; }while(0)

#define LCD_BL_GPIO_PORT                GPIOB
#define LCD_BL_GPIO_PIN                 GPIO_Pin_0
#define LCD_BL_GPIO_CLK_ENABLE()        do{ RCC->APB2ENR |= 1 << 3; }while(0)

/* LCD_CS(需要根据LCD_FSMC_NEX设置正确的IO口) 和 LCD_RS(需要根据LCD_FSMC_AX设置正确的IO口) 引脚 定义 */
#define LCD_CS_GPIO_PORT                GPIOG
#define LCD_CS_GPIO_PIN                 GPIO_Pin_12
#define LCD_CS_GPIO_CLK_ENABLE()        do{ RCC->APB2ENR |= 1 << 8; }while(0)

#define LCD_RS_GPIO_PORT                GPIOG
#define LCD_RS_GPIO_PIN                 GPIO_Pin_0
#define LCD_RS_GPIO_CLK_ENABLE()        do{ RCC->APB2ENR |= 1 << 8; }while(0)
...
/* LCD背光控制 */
#define LCD_BL(x)       do{ if(x) GPIO_SetBits(LCD_BL_GPIO_PORT, LCD_BL_GPIO_PIN); \
                            else GPIO_ResetBits(LCD_BL_GPIO_PORT, LCD_BL_GPIO_PIN); }while(0)  
...
/* SSD1963寄存器数据字节预计算 (拆分高低字节) */
#define SSD_HOR_RES_H       ((SSD_HOR_RESOLUTION - 1) >> 8)         /* 799>>8 = 0x03 */
#define SSD_HOR_RES_L       ((SSD_HOR_RESOLUTION - 1) & 0xFF)       /* 799&0xFF = 0xFF */
#define SSD_VER_RES_H       ((SSD_VER_RESOLUTION - 1) >> 8)         /* 479>>8 = 0x01 */
#define SSD_VER_RES_L       ((SSD_VER_RESOLUTION - 1) & 0xFF)       /* 479&0xFF = 0xDF */
#define SSD_HT_H            ((SSD_HT - 1) >> 8)                     /* 1055>>8 = 0x04 */
#define SSD_HT_L            ((SSD_HT - 1) & 0xFF)                   /* 1055&0xFF = 0x1F */
#define SSD_HPS_H           (SSD_HPS >> 8)                          /* 46>>8 = 0x00 */
#define SSD_HPS_L           (SSD_HPS & 0xFF)                        /* 46&0xFF = 0x2E */
#define SSD_HPW             (SSD_HOR_PULSE_WIDTH - 1)               /* 1-1 = 0x00 */
#define SSD_VT_H            ((SSD_VT - 1) >> 8)                     /* 524>>8 = 0x02 */
#define SSD_VT_L            ((SSD_VT - 1) & 0xFF)                   /* 524&0xFF = 0x0C */
#define SSD_VPS_H           (SSD_VPS >> 8)                          /* 23>>8 = 0x00 */
#define SSD_VPS_L           (SSD_VPS & 0xFF)                        /* 23&0xFF = 0x17 */
#define SSD_VFP             (SSD_VER_FRONT_PORCH - 1)               /* 22-1 = 0x15 */
