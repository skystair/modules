...
/**
 * @brief       简单延时函数(替代原delay_ms)
 * @param       nCount: 延时计数
 * @retval      无
 */
static void lcd_delay(volatile uint32_t nCount)
{
    while(nCount--);
}
...
/**
 * @brief       LCD GPIO初始化(使用STM32标准外设库替代sys_gpio_set)
 * @param       无
 * @retval      无
 */
static void lcd_gpio_init(void)
{
    ...
    /* LCD_CS(FSMC_NE4/PG12), LCD_WR(PD5), LCD_RD(PD4), LCD_RS(FSMC_A10/PG0) 复用推挽 */
    ...
    /* LCD_D0~D15: PD0,1,8,9,10,14,15 复用推挽 */
...
void lcd_init(void)
{
    lcd_gpio_init();    /* GPIO初始化(替代原来sys_gpio_set) */
    ...
    /* printf需要串口重定向支持,当前工程未实现,注释掉避免卡死 */
    /* printf("LCD ID:%x\r\n", lcddev.id); */
