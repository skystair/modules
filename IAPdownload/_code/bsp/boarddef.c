...
const BoardGpioConfig_t Board_GpioInputs[IO_INch_Max] = {
    // KEY_UP  (PA0) - 高电平有效，下拉输入，defautLV=0
    {D_PORT_A, D_PIN_(0), 0},
    // KEY_RIGHT (PE4) - 低电平有效，上拉输入，defautLV=1
    {D_PORT_E, D_PIN_(4), 1},
    // KEY_DOWN  (PE3) - 低电平有效，上拉输入，defautLV=1
    {D_PORT_E, D_PIN_(3), 1},
    // KEY_LEFT  (PE2) - 低电平有效，上拉输入，defautLV=1
    {D_PORT_E, D_PIN_(2), 1},
};
...
void gpiox_init(void){
    ...
    //in - 根据defautLV选择上拉/下拉
    for(i = 0;i < IO_INch_Max;i++){
        GPIO_str.GPIO_Pin = Board_GpioInputs[i].pin;
        if(Board_GpioInputs[i].defautLV){
            GPIO_str.GPIO_Mode = GPIO_Mode_IPU;  // 上拉输入 (KEY_RIGHT/DOWN/LEFT)
        }else{
            GPIO_str.GPIO_Mode = GPIO_Mode_IPD;  // 下拉输入 (KEY_UP)
        }
