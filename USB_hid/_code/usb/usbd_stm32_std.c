...
static int32_t USBD_EndpointConfigure(uint8_t ep_addr, uint8_t ep_type, uint16_t ep_max_packet_size) {
    ...
    if (ep_num == 0) {
        ...
        *epnr = ep_info[ep_num][1].type | (uint16_t)ep_num;
        ...
        *epnr = ep_info[ep_num][0].type | (uint16_t)ep_num;
        EPnR_SetRxStat(epnr, EP_RX_VALID);
    }
    ...
    while (1) {
        ...
        if (istr & ISTR_CTR) {
            ...
            if (ep_val & EP_CTR_TX) {
                ...
                if (ep_info[ep_id][1].configured) {
                    uint32_t pkt = ep_info[ep_id][1].num;
        ...
        if (istr & ISTR_RESET) {
            ...
            for (cnt = 0; cnt < EP_COUNT; cnt++) {
                (&USB->EP0R)[cnt * 2] = EP_DIS;
                memset(&ep_info[cnt][0], 0, sizeof(EP_Info_t));
                memset(&ep_info[cnt][1], 0, sizeof(EP_Info_t));
            }
