...
/**
 * USART1 RXNE 中断
 * 每收到1字节触发 → 读取数据寄存器 → 交给协议层 RF_433_MX_uart_rx_isr()
 */
...
/**
 * RX使能控制（REN位，非中断）
 * @param en  1=使能接收器(RE=1) 0=关闭接收器(RE=0)
 */
